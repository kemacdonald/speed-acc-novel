...
# process stimulus log xml files generated by SMI
# this maps stimulus id to stimulus metadata
process_log_files <- function(dir_path, exp_version) {
  files <- list.files(dir_path)
  files %>% purrr::map_dfr(process_log_file, file_path = dir_path, exp_version)
}

# process one stimulus log file
process_log_file <- function(file, file_path, exp_version) {
  ...
  xml_list %>% purrr::map_dfr(make_stimulus_key, order_name, order_age, exp_version)
}

# make the stimulus key from the xml object
make_stimulus_key <- function(xml_obj, order_name, order_age, exp_version) {
  # check length of xml object to tell what kind of file we are processing
  xml_len <- xml_obj %>% length()
  
  if(xml_len == 2) {
    xml_obj <- xml_obj[[".attrs"]]
  }
  ...
  # remove number from order_name for version 1 of experiment but not version 2
  # we changed the file naming convention
  if(exp_version == "v1") {
    d %>% mutate(order_name = str_remove(order_name, "_[:digit:]+"))
  } else {
    d %>% mutate(order_name = order_name)
  }
}


download_orders_gs <- function(sheet_name, exp_version, exp_order_sheet_path) {
  
  order_sheets_gs <- gs_title(sheet_name)
  
  sheet_names <- order_sheets_gs$ws$ws_title
  
  sheet_names %>% purrr::map(.f = download_one_order, 
                             gs_obj = order_sheets_gs, 
                             exp_version = exp_version, 
                             save_path = save_path)
}

download_one_order <- function(sheet_name, gs_obj, exp_version, save_path) {
  
  save_path <- file.path(exp_order_sheet_path, 
                         paste0(exp_version, "_orders"),
                         paste0("speed_acc_novel_kid_", sheet_name, "_", exp_version, ".csv"))
  
  gs_obj %>% 
    gs_download(ws = sheet_name, 
                to = save_path,
                overwrite = T)    
}
  ...
  if (nrow(crit_window_responses) <= 1) {
    ...
    trial_score <- trial_df %>% 
      ...
      mutate(shift_type = case_when(
        shift_info == "center-target" ~ "C_T",
        shift_info == "center-distracter" ~ "C_D",
        shift_info == "center-away" ~ "C_A",
        shift_info == "target-distracter" ~ "T_D",
        shift_info == "target-center" ~ "T_C",
        shift_info == "target-away" ~ "T_A",
        shift_info == "distracter-target" ~ "D_T",
        shift_info == "distracter-center" ~ "D_C",
        shift_info == "distracter-away" ~ "D_A",
        TRUE ~ "shift_not_categorized"
      ),
