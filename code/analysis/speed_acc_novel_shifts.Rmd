...
# spread out table
cond_acc_age_spread <- spread(cond_acc_age, 
                              key = shift_accuracy, 
                              value = n)
...
Add order type column to d_keep
```{r}
d_keep <- mutate(d_keep, 
                 order_type = 
...
Make table for accuracy for children based on order
```{r}
# table for accuracy by order
cond_acc_order <- d_keep %>%
  filter(age_category == "child") %>%
  group_by(gaze_condition, 
           shift_accuracy, 
           trial_type,
...
Plot overall accuracy for children based on order:
```{r}
